...
public:
    explicit Container(size_t sz) : n_elements {sz} {
        c_ptr = std::make_unique<T[]>(n_elements);
    }

    Container(initializer_list<T> i_lst) : Container(i_lst.size()) {
        std::copy(i_lst.begin(), i_lst.end(), c_ptr.get());
    }
...
void unit_tests(Container<string>& x) {
    ...
    // type. while n is unsigned this is part of the standard test
    b = x.begin() + n;
    it1 = b += -n;
    b = x.begin() + n;
    it2 = b -= n;
...
int main() {
    ...
    for (const auto& v : result) print("{} ", v);
    print("\n");
