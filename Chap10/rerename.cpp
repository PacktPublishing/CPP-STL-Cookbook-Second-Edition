// rerename.cpp by Bill Weinman [bw.org]
// as of 2026-05-16
...
int main(const int argc, const char** argv) {
    ...
    if (argc < 3 || argc % 2 != 1) {
        ...
        println("usage: {} [regex replacement] ...", cmdname);
        return 1;
    }
    ...
    for (const auto& entry : dit{fs::current_path()}) {
        ...
        if (fpath.filename().string() != rname) {
            ...
            if (exists(rpath)) {
                println("Error: cannot rename - destination file exists.");
            } else {
                fs::rename(fpath, rpath);
                println("{} -> {}", fpath.filename(), rpath.filename());
            }
        }
    }
}
