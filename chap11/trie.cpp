// trie.cpp by Bill Weinman [bw.org]
// as of 2026-05-16
...
namespace bw {
    ...
    class trie {
        ...
        void p_insert(It it, It end_it) {
            if (it == end_it) return;
            nodes[*it].p_insert(++it, end_it);
        }

        void p_get(deque<string>& dq, get_t& r_dq) const {
            if (empty()) {
            ...
            for (const auto& p : nodes) {
                ...
                p.second.p_get(dq, r_dq);
            }
        }

        void p_find_prefix(const string& s, auto& pre_dq) const {
            if (empty()) return;
            for (const auto& [k, v] : nodes) {
                if (k.starts_with(s)) {
                    pre_dq.emplace_back(k);
                    v.p_find_prefix(k, pre_dq);
                }
            }
        }
